...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        possibleLocs = util.Counter()
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for nextPos, p in newPosDist.items():
                possibleLocs[nextPos] += self.beliefs[oldPos] * p
        self.beliefs = possibleLocs
        self.beliefs.normalize()
        return
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            obsP = self.getObservationProb(observation, gameState.getPacmanPosition(), particle, self.getJailPosition())
            if particle in particleDist:
                disWeight = particleDist[particle]
                disWeight = disWeight + obsP
                particleDist[particle] = disWeight
            else:
                particleDist[particle] = obsP
        particleDist.normalize()
        if particleDist.total() > 0:
            self.particles = [particleDist.sample() for i in range(self.numParticles)]
        else: 
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        newParticles = []
        for particle in self.particles:
            particleDist = self.getPositionDistribution(gameState, particle)
            newParticles.append(particleDist.sample())
