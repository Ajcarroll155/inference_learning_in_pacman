...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            livingLocs.append(dist.argMax())
        gDists = []
        for gPos in livingLocs:
            dist = self.distancer.getDistance(pacmanPosition, gPos)
            gDists.append((dist, gPos))
        closeGhost = min(gDists)[1]
        actVals = util.Counter()
        for action in legal:
            actVals[action] = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closeGhost)
        bestAction = min(actVals, key=actVals.get)   
        return bestAction  
